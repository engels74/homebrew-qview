...
- **`gh-workflow-immortality.sh` is vendored MIT third-party code** (Daniel Rudolf, v1.1.1) inside an AGPL-3.0 repository. Update it by re-vendoring upstream, not by patching in place. Its header refers to a `THIRD_PARTY_NOTICES.md` that does not exist here; add that file rather than editing the license header out.
